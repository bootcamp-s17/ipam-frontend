	...
	ctrl.$rootScope.checkit = function (siteid) {
		if (ctrl.$rootScope.firstTime == true) {
			...
			for (var i = ctrl.$rootScope.equipments.length - 1; i > -1; i--) {
				if (ctrl.$rootScope.equipments[i].site_id == siteid) {} else {
...
function ipamService($resource) {
	...
	var getSites = function getSites() {
		return $resource('https://ipam-backend.herokuapp.com/api/sites/:site', { site: "@site" });
	};
	var addSite = function addSite() {
		return $resource('https://ipam-backend.herokuapp.com/api/sites');
	};
	...
	var getSubnets = function getSubnets() {
		return $resource('https://ipam-backend.herokuapp.com/api/subnets/:subnet', { subnet: "@subnet" });
	};
	var addSubnet = function addSubnet() {
		return $resource('https://ipam-backend.herokuapp.com/api/subnets');
	};
	var updateSubnet = function updateSubnet() {
		return $resource('https://ipam-backend.herokuapp.com/api/subnets/:subnet', { subnet: "@subnet" }, { 'update': { metod: 'PUT' }
		});
	};
	// All of the ip endpoint api functions
	var getIpBySubnet = function getIpBySubnet() {
		return $resource('https://ipam-backend.herokuapp.com/api/ip/:subnet', { subnet: "@subnet" });
	};
	var getNextIp = function getNextIp() {
		return $resource('https://ipam-backend.herokuapp.com/api/ip/:subnet/next', { subnet: "@subnet" });
	};
	var checkIp = function checkIp() {
		return $resource('https://ipam-backend.herokuapp.com/api/ip/:subnet/check/:checkIp', { subnet: "@subnet", checkIp: "@checkIp" });
	};

	// Mac Addresss endpoints
	var checkMac = function checkMac() {
		return $resource('https://ipam-backend.herokuapp.com/api/mac_address/:mac', { mac: "@mac" });
	};
	// all of the equipment api functions
	var getEquipments = function getEquipments() {
		return $resource('https://ipam-backend.herokuapp.com/api/equipment/:equipment', { equipment: "@equipment" });
	};
	var addEquipment = function addEquipment() {
		return $resource('https://ipam-backend.herokuapp.com/api/equipment');
	};
	var updateEquipment = function updateEquipment() {
		return $resource('https://ipam-backend.herokuapp.com/api/equipment/:equipment', { equipment: "@equipment" }, { 'update': { method: 'PUT' }
		});
	};
	var getEquipmentTypes = function getEquipmentTypes() {
		return $resource('https://ipam-backend.herokuapp.com/api/equipment_types');
	};
